...
This module contains the main business logic for handling technical
support agent requests following AOCC FastAPI standards.
"""
...
class TechAgentHandler:
    ...
    def __init__(
        ...
        self.chat_flow: Optional[ChatFlowStub] = None
        self.service_process: Optional[ServiceProcessStub] = None
    ...
    async def _initialize_chat(self) -> None:
        ...
        # Initialize services
        self.service_process = ServiceProcessStub(
            system_code=self.user_input.system_code,
            container=self.container,
        )
        self.chat_flow = ChatFlowStub(
    ...
    async def _get_user_and_scope_info(self) -> None:
        ...
        # Use gather to wait in parallel
    ...
    async def run(self, log_record: bool = True) -> Dict[str, Any]:
        ...
        if log_record:
            cosmos_data = await self._log_and_save_results()
            set_extract_log(
                {
                    "exec_time": cosmos_data.get("total_time", 0),
                    "response_type": self.response_data.get("type", ""),
                    "bot_scope": self.bot_scope_chat,
