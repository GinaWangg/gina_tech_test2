...
class TechAgentHandler:
    ...
    async def _process_history(self) -> None:
        ...
        # results_related = await self.container.sentence_group_classification.sentence_group_classification(
        #     self.his_inputs
        # )
        results_related = await self.container.sentence_group_classification.sentence_group_classification(
            self.his_inputs
    ...
    async def _get_user_and_scope_info(self) -> None:
        ...
        if not self.bot_scope_chat and self.user_info_dict.get(
    ...
    async def _handle_high_similarity(self) -> None:
        ...
        """
        logger.info(
            f"[高相似度] 相似度={self.top1_kb_sim}, KB={self.top1_kb}"
        )
        ...
        if self.user_input.system_code.lower() == "rog":
            link = f"https://rog.asus.com/{self.user_input.websitecode}/support/FAQ/{self.top1_kb}"
        else:
            link = f"https://www.asus.com/{self.user_input.websitecode}/support/FAQ/{self.top1_kb}"
