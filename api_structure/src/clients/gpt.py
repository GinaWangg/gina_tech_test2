...
class GptClient:
    ...
    def __init__(
        ...
        Raises:
            ValueError: If required credentials are not provided or found in
                environment.
        """
        self._resource_endpoint = resource_endpoint or os.getenv(
            "MYAPP_GPT4O_RESOURCE_ENDPOINT", ""
        )
        self._api_key = api_key or os.getenv("MYAPP_GPT4O_API_KEY", "")
        self._default_model = default_model or os.getenv("MYAPP_GPT4O_INTENTDETECT", "")
    ...
    def _validate_credentials(self) -> None:
        ...
        """
        if not self._resource_endpoint or not self._api_key or not self._default_model:
            raise ValueError(
    ...
    def _to_json_format(self, x):
        ...
        if not isinstance(x, dict):
            try:
                x = json.loads(
                    x.replace("jsonl", "").replace("```", "").replace("json", "")
                )
    ...
    async def _call(
        ...
        try:
            response = await asyncio.wait_for(gpt_chat_completion(), timeout=timeout)
            return self._to_json_format(response)

        except asyncio.TimeoutError:
            print(
                f"GPT-4 call timed out after {timeout}s, retrying without " f"timeout"
            )
    ...
    async def call_with_prompts(
        ...
        return await self._call(
            conversation, timeout=timeout, temperature=temperature, model=model
        )
    ...
    async def call_with_functions(
        ...
        Args:
            ...
            function_call: Specification of which function to call.
        ...
        Returns:
            The message object containing function_call information.
        ...
        try:
            response = await asyncio.wait_for(gpt_function_call(), timeout=timeout)
            return response

        except asyncio.TimeoutError:
            print(
                f"GPT-4 function call timed out after {timeout}s, retrying "
                f"without timeout"
            )
            response = await gpt_function_call()
            return response
